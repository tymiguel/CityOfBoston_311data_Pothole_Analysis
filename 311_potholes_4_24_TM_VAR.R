...
co_arima1 <- auto.arima(4, ic = "aic", trace = T)
...
##### FINAL PREDICTIONS with MARCH ####
##### all data and constituent only ####

# get march data #
march <- read_csv("/Users/tylermiguel/Desktop/311 Service Requests/311._with_March.csv")
march %>%
  dplyr::filter(CASE_TITLE == 'Request for Pothole Repair') %>%
  {.} -> march_ph
## Remove duplicate cases
...
# create TS
march_prep <- march_clean[,2] # extract time (when request is made)
march_prep <- format(march_prep$open_dt, "%Y-%m") 
march_prep <- data.frame(date = march_prep) # create a dataframe
march_prep %>%
  group_by(date) %>%
  summarise(count = n()) %>%
  {.} -> march_prep # just counting how pot hole requests we have per month

#### Create a time series ####
# build the series
# check stationarity
# settle on data model to move forward with
march_ph_ts <- ts(march_prep$count, start = c(2011, 7), frequency = 12)
march_count <- window(march_ph_ts, start = c(2017, 3), frequency = 12)

# final model predictions for all data using ARIMA(1,1,1,1,1,1)12
sqrt(mean((arima7$residuals)^2)) # 198 RMSE for training model
val_arima7 <- predict(arima7, 5)
sqrt(mean((monthly_ts_test - val_arima7$pred)^2)) # 446

test_arima7 <- predict(arima7, 7)
march_count[1]-test_arima7$pred[6] # off by 239 for march

# plot of the predictions
plot(as.numeric(monthly_ts_test), type="l")
lines(test_arima7$pred[1:5], type='l', col="blue")
lines(test_arima7$pred[1:5]+test_arima7$se[1:5]*1.96, type='l', col="red", lty = 2)
lines(test_arima7$pred[1:5]-test_arima7$se[1:5]*1.96, type='l', col="red", lty=2)

# final model predictions for constient data using ARIMA(1,1,1,1,1,1)12
march_clean %>%
  dplyr::filter(Source == "Citizens Connect App" |
                  Source == "Constituent Call" |
                  Source == "Twitter") %>%
                  {.} -> constituents_only_march
co_march <- constituents_only_march[,2] # extract time (when request is made)
co_march <- format(co_march$open_dt, "%Y-%m") # turns POSIXct YYYY-MM-DD HH:MM:SS to character vector YYYY-MM format
co_march <- data.frame(date = co_march) # create a dataframe
co_march %>%
  group_by(date) %>%
  summarise(count = n()) %>%
  {.} -> co_march # just counting how pot hole requests we have per month
# build the series
# check stationarity
# settle on data model to move forward with
co_march_ts <- ts(co_march$count, start = c(2011, 7), frequency = 12)
co_march_only_ts <- window(co_march_ts, start = c(2017, 3), frequency = 12) 

co_march_only_ts

co_arima7 = arima(co_monthly_ts_train, order=c(1,1,1),seasonal=list(order=c(1,1,1),period=12))
arima7 #aic = 614

co_val_arima7 <- predict(co_arima7, 7)
sqrt(mean((co_monthly_ts_test - co_val_arima7$pred[1:5])^2)) # RMSE 123
co_march_only_ts[1] - co_val_arima7$pred[6] # 409 vs 519 expected

# plot of the predictions of constiuents only
plot(as.numeric(co_monthly_ts_test), type="l")
lines(co_val_arima7$pred[1:5], type='l', col="blue")
lines(co_val_arima7$pred[1:5]+co_val_arima7$se[1:5]*1.96, type='l', col="red", lty = 2)
lines(co_val_arima7$pred[1:5]-co_val_arima7$se[1:5]*1.96, type='l', col="red", lty=2)
...
test_range_dates <- history_daily_range(location, 
                                         from_date = "2014-05-03",
                                         to_date = "2015-09-15")

as.Date("2015-09-15") - 500

weather <- bind_rows(test_range_dates, weather)

write_csv(weather, "weather_updated.csv")
...
new_weather$date <- format(new_weather$date, "%Y-%m")
weather_prepared <- aggregate(new_weather, list(new_weather$date), mean)
min_temp <- weather_prepared[c("Group.1","snow")]
min_temp_ts <- ts(min_temp[,2], start = c(2014,5), frequency = 12)
min_temp_train <- window(min_temp_ts, start = c(2014,9), end = c(2016,9))
min_temp_test <- window(min_temp_ts, start = c(2016,10), end = c(2017,2))
plot(min_temp_train)
plot(min_temp_test)

# make window on potholes
monthly_ts_train_VAR <- window(monthly_ph_ts, start = c(2014, 5), end = c(2016,9) , frequency = 12) # take a window on time from July 2011 to December 2016.
...
snow_var_ts <- ts(last_var[,2], start = c(2014,5), frequency = 12)
mean_pressure_var_ts <- ts(last_var[,3], start = c(2014,5), frequency = 12)
mean_wind_spd_ts <- ts(last_var[,4], start = c(2014,5), frequency = 12)
mean_visib_ts <- ts(last_var[,5], start = c(2014,5), frequency = 12)

snow_var_train <- window(snow_var_ts, start = c(2014,5), end = c(2016,9))
mean_pressure_var_train <- window(mean_pressure_var_ts, start = c(2014,5), end = c(2016,9))
mean_wind_spd_train <- window(mean_wind_spd_ts, start = c(2014,5), end = c(2016,9))
mean_visib_train <- window(mean_visib_ts, start = c(2014,5), end = c(2016,9))
...
VARselect(X_2, lag.max = 5, type = "const")

summary(VAR_model_2)
plot(VAR_model_2)

# check stabiliy of VAR model
roots(VAR_model_2) # first eigenvalue is smaller than unity

AIC(VAR_model_2$varresult$monthly_ts_train_VAR) # 365
VAR_model_2$varresult$monthly_ts_train_VAR

sqrt(mean((monthly_ts_train_VAR[1:28]-fitted(VAR_model_2)[,"monthly_ts_train_VAR"])^2)) # 337 RMSE for model
pred_VAR_2 = predict(VAR_model_2,n.ahead = 5)
sqrt(mean((monthly_ts_test-pred_VAR_2$fcst$monthly_ts_train_VAR[,"fcst"])^2)) # 639.87 RMSE for predictions look much better!
...
pred_VAR_2$fcst$monthly_ts_train_VAR

#### VAR lag 2 model #####

# do PCA and then time series

set.seed(2101)
prcomp <- prcomp(weather_prep_final[2:12], scale = T)
prcomp$x[, 1:2]
pr_comp <- prcomp$x[, 1]

pr_ts <- ts(pr_comp, start = c(2014, 5), end = c(2017,2) , frequency = 12)
pr_ts_train <- ts(pr_comp, start = c(2014, 5), end = c(2016,9) , frequency = 12)
pr_ts_test <- ts(pr_comp, start = c(2016, 10), end = c(2017,2) , frequency = 12)

X_3 <- cbind(monthly_ts_train_VAR,pr_ts_train)

plot(X_3)

VAR_model_3 <- VAR(X_3, p=1, type="both")
VARselect(X_3, lag.max = 5, type = "const")

summary(VAR_model_3)
plot(VAR_model_3)

# check stabiliy of VAR model
roots(VAR_model_3) # first eigenvalue is smaller than unity

AIC(VAR_model_3$varresult$monthly_ts_train_VAR) # 387
VAR_model_3$varresult$monthly_ts_train_VAR

sqrt(mean((monthly_ts_train_VAR[1:28]-fitted(VAR_model_3)[,"monthly_ts_train_VAR"])^2)) # 294 RMSE for model
pred_VAR_3 = predict(VAR_model_3,n.ahead = 5)
sqrt(mean((monthly_ts_test-pred_VAR_3$fcst$monthly_ts_train_VAR[,"fcst"])^2)) # 765 RMSE for predictions look much better!
